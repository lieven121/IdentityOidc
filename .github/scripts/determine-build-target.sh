...
if [[ "${GITHUB_REF}" == refs/tags/* ]]; then
  ...
  if [ "$BRANCH_NAME" == "dev" ]; then
    ...
    IMAGE_TAG=("type=raw,value=dev-${DATE_TAG},enable=true" "type=raw,value=dev,enable=true")
    ...
    if git diff-tree --no-commit-id --name-only -r HEAD | grep -q '^VERSION'; then
      ...
      VERSION=$(cat VERSION)
      IMAGE_TAG=("type=raw,value=${VERSION}-rc,enable=true" "type=raw,value=latest-rc,enable=true")
    ...
    if git diff-tree --no-commit-id --name-only -r HEAD | grep -q '^VERSION'; then
      echo "VERSION file was updated in this commit."
      VERSION=$(cat VERSION)
      IMAGE_TAG=("type=raw,value=${VERSION},enable=true" "type=raw,value=latest,enable=true")
